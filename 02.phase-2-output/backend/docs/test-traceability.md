...
| STORY-005 Account update | `WorkflowIntegrationTest.rejectsStaleAccountSnapshotWithoutWritingEitherRecord`; `LegacyValidationServiceTest.accountAndCustomerMaintenance...` | Version conflict prevents writes; account/customer formats, DOB, FICO and state/ZIP boundaries. |
| STORY-006 Card list | `ApiContractIntegrationTest.accountAndCardEndpointsCoverStories004Through008`; `LegacyValidationServiceTest.accountAndCardIdentifiers...` | Pagination, account filtering, invalid card/account filters. |
...
| STORY-008 Card update | `ApiContractIntegrationTest.accountAndCardEndpointsCoverStories004Through008`; `LegacyValidationServiceTest.cardMaintenance...` | Valid update, no-change rejection, name/status/month/year validation boundaries. |
| STORY-009 Transaction browse | `ApiContractIntegrationTest.transactionEndpointsCoverStories009Through011IncludingNegativeValidation`; `LegacyValidationServiceTest.menuAndRowSelections...` | Paging, numeric filter validation, selection action validation. |
...
| STORY-011 Add transaction | `ApiContractIntegrationTest.transactionEndpointsCoverStories009Through011IncludingNegativeValidation`; `WorkflowIntegrationTest.createsRelatedTransactionAndAllocatesMonotonicUniqueIds`; `LegacyValidationServiceTest.confirmationsAndTransactionFields...` | Confirmed creation, no-create confirmation gate, calendar/decimal failures, required fields, relationship failure, monotonically allocated unique IDs. |
| STORY-012 Bill payment | `ApiContractIntegrationTest.paymentAndUserAdministrationCoverStories012Through016`; `PaymentRulesServiceTest`; `WorkflowIntegrationTest.rollsBackPaymentWhenPaymentConfigurationIsMissing`; `WorkflowIntegrationTest.savesPaymentAndReportRequestAsRetrievableWorkflowState` | Positive/zero/negative balance boundaries, full settlement, payment transaction, confirmation error, and transaction rollback. |
...
| STORY-014 Add user | `ApiContractIntegrationTest.paymentAndUserAdministrationCoverStories012Through016`; `UserControllerTest`; `LegacyValidationServiceTest.userAndSignOnInputs...` | Create success, required values, and duplicate key conflict. |
| STORY-015 Update user | `ApiContractIntegrationTest.paymentAndUserAdministrationCoverStories012Through016` | Persisted user changes; service no-change behavior is covered in the workflow service suite. |
| STORY-016 Delete user | `ApiContractIntegrationTest.paymentAndUserAdministrationCoverStories012Through016` | Explicit delete then not-found retrieval. |
...
| STORY-018 Report output | `ApiContractIntegrationTest.reportLifecycleAndOpenApiCoverStories017And018`; `WorkflowIntegrationTest.savesPaymentAndReportRequestAsRetrievableWorkflowState` | Inclusive date-filtered/card-sorted output and empty selected range. |
...
The app seeds the supplied immutable fixed-width legacy extracts: 50 accounts/customers/cards/assignments, 300 transactions, configuration data, and `ADMIN001` / `USER0001`. The API integration suite uses account `1` and card `0500024453765740`, then creates/deletes `QAUSER01`; it resets the seed baseline before use. Manual test data and reset instructions are in the README.
