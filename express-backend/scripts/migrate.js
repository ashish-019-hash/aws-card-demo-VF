...
export async function runMigrations({ pool = createPool(), migrationsDir = defaultMigrationsDir, log = console.log } = {}) {
  return withTransaction(pool, async client => {
    // F-006: transaction-scoped PostgreSQL lock serializes runners; this transaction-wide approach precludes CREATE INDEX CONCURRENTLY migrations.
    await client.query("SELECT pg_advisory_xact_lock(hashtext('carddemo-schema-migrations'))");
